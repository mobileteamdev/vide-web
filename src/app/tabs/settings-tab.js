...
module.exports = class SettingsTab {
  constructor (localRegistry) {
    ...
    self._view = { /* eslint-disable */
      ...
      theme: { dark: null, light: null, vtho: null },
      plugins: {},
      config: {
    ...
    self.data.currentTheme = self._components.themeStorage.get('theme') || 'vtho'
  }
  render () {
    const self = this
    ...
    self._view.theme.vtho = yo`<input onchange=${onswitch2vthoTheme} class="${css.col1}" name="theme" id="themeVTHO" type="radio">`
	...
	self._view.config.general_Personal = yo`<div class="${css.crow}"><div>${self._view.personal}></div><span class="${css.checkboxText}">Enable Personal Mode ${self._view.warnPersonalMode}></span></div>`
    ...
    self._view.config.general = yo`
      <div class="${css.info}">
          ...
          <div class="${css.crow}">
            ...
            <span class="${css.checkboxText}">Generate contract metadata. Generate a JSON file in the contract folder. Allows to specify library addresses the contract depends on. If nothing is specified, VIDE deploy libraries automatically.</span>
          </div>
          <div class="${css.crow}">
    ...
    self._view.gistToken = yo`
      <div class="${css.info}">
        ...
        <div class="${css.crowNoFlex}">Go to github token page (link below) to create a new token and save it in VIDE. Make sure this token has only 'create gist' permission.</div>
    ...
    self._view.config.themes = yo`
      <div class="${css.info}">
        ...
        <div class="${css.crow}">
          ${self._view.theme.vtho}
          <label for="themeVTHO">VTHO Theme</label>
        </div>
        <div class="${css.crow}">
    ...
    function onswitch2vthoTheme (event) {
      styleGuide.switchTheme('vtho')
