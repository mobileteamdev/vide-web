...
const copy = require('copy-text-to-clipboard') // remove 'clipboard-copy'
...
module.exports = class CompileTab {
  ...
  render () {
    ...
    function copyABI (event) {
    	event.stopPropagation()
      copyContractProperty('abi')
    }
    function copyBytecode (event) {
    	event.stopPropagation()
  ...
  _updateVersionSelector () {
    ...
    self.data.allversions.forEach(build => {
		if(window.fixedLatestRelease){
			var numVs1 = Number(fixedLatestRelease[0]+fixedLatestRelease[2]+fixedLatestRelease[4])
			var numVs2 = Number(build.version[0]+build.version[2]+build.version[4])
			if(numVs2 > numVs1)return;
		}
		self._view.versionSelector.appendChild(yo`<option value=${build.path}>${build.longVersion}</option>`)
	})
    ...
    var isFirefox = typeof InstallTrigger !== 'undefined' || navigator.userAgent.indexOf(' Sync/')!==-1
    ...
    minixhr(`${self.data.baseurl}/list.json`, function (json, event) {
      ...
      if (event.type !== 'error') {
        try {
		  ...
		  if(window.fixedLatestRelease)data.latestRelease = fixedLatestRelease;
          selectedVersion = data.releases[data.latestRelease]
          if (self._components.queryParams.get().version){////vide
		  		if(window.fixedLatestRelease){
					var queryVs = self._components.queryParams.get().version;
					var numVs1 = Number(fixedLatestRelease[0]+fixedLatestRelease[2]+fixedLatestRelease[4])
					var numVs2 = Number(queryVs[9]+queryVs[11]+queryVs[13])//soljson-v0.5.6+commit.b259423e.js
					if(numVs2 < numVs1)selectedVersion = queryVs;
				} else{
			  		selectedVersion = self._components.queryParams.get().version
				}
		  }
        } catch (e) {
