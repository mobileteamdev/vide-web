...
class Terminal {
  constructor (opts, api) {
	...
	self.ideNamespace = window.ideNamespace || 'remix'
    ...
    self._components.dropdown = new Dropdown({
      options: [
        'only '+self.ideNamespace+' transactions',
        'all transactions',
        'script'
      ],
      defaults: ['only '+self.ideNamespace+' transactions', 'script'],
      dependencies: {'all transactions': ['only '+self.ideNamespace+' transactions'], ['only '+self.ideNamespace+' transactions']: ['all transactions']}
    })
  ...
  render () {
    ...
    var intro = yo`<div><div> - Welcome to VIDE v0.0.1 - </div><br>
                  ...
                  <ul class=${css2.ul}>
                    ...
                    <li>Running JavaScript scripts. The following libraries are accessible:
                      <ul class=${css2.ul}>
                        <li><a target="_blank" href="https://connex.vecha.in/">connex</a></li>
                    ...
                    <li>Executing common command to interact with the VIDE interface (see list of commands above). Note that these commands can also be included and run from a JavaScript script.</li>
...
function domTerminalFeatures (self, scopedCommands) {
  return {
    ...
    [self.ideNamespace]: self._components.cmdInterpreter,
