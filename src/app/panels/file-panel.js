...
function filepanel (localRegistry) {
      ...
      <div class=${css.container}>
        <div class=${css.fileexplorer}>
          <div class=${css.menu}>
            ...
            <span class="${css.copyFiles}" title="Copy all files to another instance of VIDE" onclick=${copyFiles}>
  ...
  function connectToLocalhost () {
		  modalDialogCustom.alert("This feature will be available in the next version");
		  return;
  ...
  function toGist (fileProviderName, id) {
    packageFiles(self._deps.fileProviders[fileProviderName], (error, packaged) => {
      if (error) {
        ...
        if (!tokenAccess) {
          modalDialogCustom.alert((/*xxx*/window.ideNamespace ? ideNamespace.toUpperCase() : 'Remix')+' requires an access token (which includes gists creation permission). Please go to the settings tab for more information.')
        } else {
