...
class TxLogger {
	constructor (localRegistry) {
		...
		//@only for vet
		this.logNewBlock = this._deps.editorPanel.registerCommand('newBlock', (args, cmds, append) => {
			var data = args[0]
			append(yo`
				<span class=${css.txLog}>
				  <span class='${css.tx}'><div class=${css.txItem}>[<span class=${css.txItemTitle}>block:${data.block.number} - </span> ${data.block.transactions.length} transactions **will display transactions details in the next version]</span></span>
				</span>`)
		}, { activate: true })
...
function txDetails (e, tx, data, obj) {
	...
	if (table && table.parentNode) {
		...
		table = createTable({
			...
			contractAddress: data.receipt ? data.receipt.contractAddress : null,
			data: data.tx,
			from,
			to,
			gas: data.tx.gas,
			...
			gasPayer: data.receipt ? data.receipt.gasPayer : null
		})
...
function createTable (opts) {
	...
	var callWarning = ''
	if(opts.isCall)callWarning = '(Cost only applies when called by a contract)';
	...
	if(opts.gasPayer){
		table.appendChild(yo`
			<tr class="${css.tr}">
				<td class="${css.td}"> gas payer </td>
				<td class="${css.td}">${opts.gasPayer}
					${copyToClipboard(() => opts.gasPayer)}
				</td>
			</tr>`)
	}
