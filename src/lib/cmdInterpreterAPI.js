...
var RemixDebug = require('../../vide_modules/vide-debug').EthDebugger
...
class CmdInterpreterAPI {
  constructor (terminal, localRegistry) {
    ...
    self.commandHelp = {
      'vide.getFile(path)': 'Returns the content of the file located at the given path',
      'vide.setFile(path, content)': 'set the content of the file located at the given path',
      'vide.debug(hash)': 'Start debugging a transaction.',
      'vide.loadgist(id)': 'Load a gist in the file explorer.',
      'vide.loadurl(url)': 'Load the given url in the file explorer. The url can be of type github, swarm, ipfs or raw http',
      //'vide.setproviderurl(url)': 'Change the current provider to Web3 provider and set the url endpoint.',
      'vide.execute(filepath)': 'Run the script specified by file path. If filepath is empty, script currently displayed in the editor is executed.',
      'vide.exeCurrent()': 'Run the script currently displayed in the editor',
      'vide.help()': 'Display this help message',
      'vide.debugHelp()': 'Display help message for debugging'
    }
  }
  ...
  debugHelp (cb) {
    const self = this
    var help = yo`<div>Here are some examples of scripts that can be run (using ${self._components.terminal.ideNamespace}.exeCurrent() or directly from the console)</div>`
    help.appendChild(yo`<br>`)
    help.appendChild(yo`<br>`)
    help.appendChild(yo`<div>${self._components.terminal.ideNamespace}.debug('0x3c247ac268afb9a9c183feb9d4e83df51efbc8a2f4624c740789b788dac43029', function (error, debugSession) {
     ${self._components.terminal.ideNamespace}.log = function () { arguments[0] != null ? console.log(arguments[0]) : console.log(arguments[1]) }

     ${self._components.terminal.ideNamespace}.d.traceManager.getLength(${self._components.terminal.ideNamespace}.log)
     ${self._components.terminal.ideNamespace}.storageView = ${self._components.terminal.ideNamespace}.d.storageViewAt(97, '0x692a70d2e424a56d2c6c27aa97d1a86395877b3a')
     console.log('storage at 97 :')
     ${self._components.terminal.ideNamespace}.storageView.storageRange(${self._components.terminal.ideNamespace}.log)
})<br/></div>`)
    help.appendChild(yo`<div>${self._components.terminal.ideNamespace}.log = function () { arguments[0] != null ? console.log(arguments[0]) : console.log(arguments[1]) }
    ${self._components.terminal.ideNamespace}.d.extractStateAt(2, function (error, state) {
        ${self._components.terminal.ideNamespace}.d.decodeStateAt(97, state, ${self._components.terminal.ideNamespace}.log)
    })<br/></div>`)
    help.appendChild(yo`<br>`)
    help.appendChild(yo`<div>${self._components.terminal.ideNamespace}.highlight(contractAddress, vmTraceIndex)</div>`)
    help.appendChild(yo`<br>`)
    help.appendChild(yo`<div>${self._components.terminal.ideNamespace}.goTo(row) (this log the index in the vm trace, state and local variables)</div>`)
    help.appendChild(yo`<br>`)
    help.appendChild(yo`<div>${self._components.terminal.ideNamespace}.stateAt(vmTraceIndex)</div>`)
    help.appendChild(yo`<br>`)
    help.appendChild(yo`<div>${self._components.terminal.ideNamespace}.localsAt(vmTraceIndex)</div>`)
    help.appendChild(yo`<br>`)
    self._components.terminal.commands.html(help)
    if (cb) cb()
    return ''
