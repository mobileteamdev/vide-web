...
class EventsDecoder {
	...
	*/
	...
	//@
	_decodeLogs (tx, receipt, contractName, compiledContracts, cb) {
		if(!contractName || !receipt)return cb('cannot decode logs - contract or receipt not resolved ');
		if(!receipt.logs)return cb(null, { decoded: [], raw: [] });
		this._decodeEvents(tx, receipt.logs, contractName, compiledContracts, cb)
	}
	//@
	_eventABI (contract) {
		var eventABI = {}
	...
	_event (hash, eventsABI) {
		for (var k in eventsABI) {
			if(eventsABI[k][hash])return eventsABI[k][hash]
		}
		return null
	}
	//@
