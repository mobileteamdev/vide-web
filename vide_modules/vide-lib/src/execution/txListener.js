...
class TxListener {
	constructor (opt) {
		...
		opt.event.udapp.register('callExecuted', (error, from, to, data, lookupOnly, txResult) => {
			...
			this._resolveTx(call, call, (error, resolvedData) => {
				if(!error)this.event.trigger('newCall', [call])
			})
		})
    ...
    */
	...
	setListenOnNetwork(listenOnNetwork){
		...
		if(this._listenOnNetwork)this._startListenOnNetwork()
	}
    ...
    */
    ...
    _startListenOnNetwork () {
		this._loopId = setInterval(() => {
			if(!window.connex || !connex.thor || !connex.thor.status || !connex.thor.status.head || !connex.thor.status.head.number)return;
			var blockNumber = connex.thor.status.head.number;
	...
	_manageBlock (blockNumber) {
		connex.thor.block(blockNumber).get().then(result=>{
			result&&this._newBlock(Object.assign({type: 'web3'}, result))
		})
	}
		...
		async.each(transactions, (tx, cb) => {
			this._api.resolveReceipt(tx, (error, receipt) => {
				...
				this._resolveTx(tx, receipt, (error, resolvedData) => {
					if (error) cb(error)
					if (resolvedData) {
						this.event.trigger('txResolved', [tx, receipt, resolvedData])//currently not used
					}
	...
	_resolveTx (tx, receipt, cb) {
		...
		if (!tx.to || tx.to === '0x0') { // testrpc returns 0x0 in that case
			...
			if(contractName) {
				...
				connex.thor.account(tx.to).getCode().then(code=>{
					if(code && code.code) {
						var contractName = this._tryResolveContract(code.code, contracts, false)
						if(contractName){
